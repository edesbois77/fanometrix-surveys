-- Migration 005: Add is_demo flag to responses
-- Run in: supabase.com → your project → SQL Editor → New query → Run

-- Add is_demo column (false by default — existing real data is unaffected)
alter table responses
  add column if not exists is_demo boolean not null default false;

-- Index for fast demo data filtering and deletion
create index if not exists idx_responses_is_demo on responses (is_demo)
  where is_demo = true;

-- Update reporting view to expose is_demo for Looker Studio filtering
...
select
  ...
  r.is_demo,                                        -- ← new: filter demo data in Looker Studio
