...
import {
  RESEARCH_GOALS, FREQUENCIES, SEARCH_STATUSES,
  KEYWORD_TYPES, PLATFORMS, MARKETS, detectKeywordType,
  RESEARCH_GOAL_LABELS, SOURCE_DESCRIPTIONS,
} from "@/lib/social-taxonomy";

// What insight each research goal delivers — written for the researcher (what
// you'll learn), not what the system does. Keyed by the stored value.
const RESEARCH_GOAL_HELP: Record<string, string> = {
  "Fan Sentiment": "Measures overall fan sentiment, identifies the positive and negative themes, and explains what's driving those opinions.",
  "Emerging Topics": "Surfaces the new and rising themes fans are talking about, so you spot shifts in the conversation early.",
  "Sponsorship Perception": "Shows how fans perceive the sponsorship — where it resonates, where it doesn't, and what shapes that view.",
  "Market Comparison": "Compares how fan opinion differs across your selected markets, highlighting where views diverge.",
  "Custom": "Analyses the conversation broadly against your research question, without a fixed lens.",
};

// Short phrase used in the live Research Summary.
const RESEARCH_GOAL_PHRASE: Record<string, string> = {
  "Fan Sentiment": "Analyse fan opinion",
  "Emerging Topics": "Identify emerging themes",
  "Sponsorship Perception": "Evaluate sponsorship impact",
  "Market Comparison": "Compare opinion across markets",
  "Custom": "Analyse the conversation",
};

// Collection Size presets — friendly sizing over the YouTube numeric limits;
// the exact numbers stay available for power users beneath.
const COLLECTION_SIZES: { key: string; label: string; max_videos: number; comments_per_video: number }[] = [
  { key: "small",  label: "Small",  max_videos: 10, comments_per_video: 10 },
  { key: "medium", label: "Medium", max_videos: 25, comments_per_video: 25 },
...
export function SearchConfigForm({ mode, searchId, backHref, backLabel }: {
  ...
  // Live research summary — reflects the brief back to the user before saving.
  const joinAnd = (arr: string[]) => (arr.length <= 1 ? (arr[0] ?? "") : `${arr.slice(0, -1).join(", ")} and ${arr[arr.length - 1]}`);
  const summaryMarketNames = form.markets.map(c => MARKETS.find(m => m.code === c)?.label ?? c);
  const summaryWindow = COLLECTION_WINDOWS.find(w => w.value === form.collect_window)?.label ?? "";
  const summaryBullets: string[] = [];
  if (form.platforms.length) summaryBullets.push(`Search ${joinAnd(form.platforms)} conversations`);
  if (form.markets.length) summaryBullets.push(`In ${joinAnd(summaryMarketNames)}`);
  if (form.collect_window === "custom") {
    if (form.collect_from || form.collect_to) summaryBullets.push(`Between ${form.collect_from || "…"} and ${form.collect_to || "…"}`);
  } else if (summaryWindow) {
    summaryBullets.push(`Across the ${summaryWindow.toLowerCase()}`);
  }
  if (keywords.length) summaryBullets.push(`Looking for: ${keywords.map(k => k.keyword).join(", ")}`);
  summaryBullets.push(RESEARCH_GOAL_PHRASE[form.research_goal] ?? "Analyse the conversation");
      ...
      <PageContainer>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div>
              ...
              <select value={form.research_goal} onChange={e => setForm(f => ({ ...f, research_goal: e.target.value }))}
                ...
                {RESEARCH_GOALS.map(g => <option key={g} value={g}>{RESEARCH_GOAL_LABELS[g] ?? g}</option>)}
              </select>
        ...
        <Card>
          ...
          <div className="mt-5">
            ...
            <p className="text-[11px] mt-1.5 leading-relaxed" style={{ color: "var(--text-tertiary)" }}>Tell Fanometrix the words people are likely to use when discussing your research subject — brands, competitions, clubs, hashtags, campaigns or common abbreviations. We detect each keyword&apos;s type automatically; click a type to change it.</p>
            {keywords.length > 0 && (
        ...
        <Card>
          ...
          <div className="mt-5 space-y-5">
            ...
            <div>
              ...
              <p className="text-[11px] mt-2" style={{ color: "var(--text-tertiary)" }}>Fanometrix searches conversations published within the selected markets.</p>
            </div>
            <div>
        ...
        {/* 4 — How should Fanometrix collect evidence? */}
        <Card>
          <SectionHeading title="How should Fanometrix collect evidence?" />
          <div className="mt-5 space-y-5">
            {/* Time period */}
            <div>
            ...
            <div>
              ...
              {form.frequency === "Manual" ? (
                ...
                <div className="mt-2 flex items-start gap-2 px-3 py-2" style={{ borderRadius: "var(--radius-control)", background: "#FBF3E1", border: "1px solid #ECDCB8" }}>
                  <span aria-hidden className="text-xs leading-none mt-0.5" style={{ color: "#8A6A2F" }}>⚠️</span>
                  <p className="text-[11px] leading-relaxed" style={{ color: "#8A6A2F" }}>
                    This search will continue collecting new conversations until paused or archived. Automatic collection increases API usage and stored evidence.
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Advanced source settings — connector-specific only, collapsed by default */}
                      ...
                      <div key={conn.id}>
                        ...
                        {conn.id === "youtube" ? (
                          <div className="space-y-3">
                            <div>
                              <FieldLabel>Collection Size</FieldLabel>
                              <div className="flex flex-wrap gap-1.5">
                                {COLLECTION_SIZES.map(s => {
                                  const yc = form.connector_config.youtube ?? {};
                                  const selected = Number(yc.max_videos) === s.max_videos && Number(yc.comments_per_video) === s.comments_per_video;
                                  return <FilterChip key={s.key} label={s.label} selected={selected}
                                    onClick={() => { setConnectorConfig("youtube", "max_videos", s.max_videos); setConnectorConfig("youtube", "comments_per_video", s.comments_per_video); }} />;
                                })}
                              </div>
                              <p className="text-[11px] mt-1.5" style={{ color: "var(--text-tertiary)" }}>Larger collections gather more conversations but take longer to run.</p>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* Research Summary — reflects the brief back before saving */}
        <Card padding="md">
          <SectionHeading title="Research Summary" />
          <div className="mt-4">
            {form.description.trim() && (
              <div className="mb-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.06em]" style={{ color: "var(--text-tertiary)" }}>Research Question</p>
                <p className="text-sm mt-0.5" style={{ color: "var(--text-primary)" }}>{form.description.trim()}</p>
              </div>
            )}
            <p className="text-sm font-semibold mb-2" style={{ color: "var(--text-primary)" }}>Fanometrix will:</p>
        ...
        {/* Where the research runs next — keeps the separation explicit. */}
        <p className="text-xs px-1" style={{ color: "var(--text-tertiary)" }}>
          Collecting evidence and reviewing results happens in{" "}
