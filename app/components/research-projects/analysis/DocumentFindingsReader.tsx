...
// Shared, CHROMELESS Document Findings reader (the two-layer document output:
// the project-specific interpretation over the approved Research Library
// analysis). Extracted from reports/document/[evidenceId] so the identical
// reader mounts in two hosts: reports/document/[evidenceId] (AdminShell; Product
// Walkthrough re-exports it, unchanged) and (workspace)/analysis/document/[evidenceId]
// (Research Project workspace shell). Review logic, engines, approval workflow
// and stored outputs are untouched — only the chrome is host-supplied.
//
// One deliberate detail preserved: `evidenceId` here is the
// research_project_evidence row's own id, not the document's library id —
// research_summaries.source_id for Document Intelligence is the evidence row id
// (migration 102). See analyseDocumentForProject.ts for the reasoning.
...
export function DocumentFindingsReader() {
  ...
  // Keyed by the evidence ROW's own id (params.evidenceId), never evidence_id.
      ...
      <div className="p-4 md:p-6 max-w-4xl mx-auto">
        ...
        {!generating && row && current && (
          <div className="space-y-6">
            ...
            <ReportHero
              variant="flat"
              kicker={`${documentTypeLabel(doc.document_type)} · ${new Date(row.generated_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}`}
