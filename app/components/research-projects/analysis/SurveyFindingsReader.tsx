...
// Shared, CHROMELESS Survey Findings reader. Extracted from the original
// reports/survey/[evidenceId] page so the identical reader mounts in two hosts:
//   • reports/survey/[evidenceId]  → wrapped in AdminShell (Product Walkthrough
//     re-exports this page; its behaviour is unchanged).
//   • (workspace)/analysis/survey/[evidenceId] → mounted inside the Research
//     Project workspace shell.
// The review logic (adapter + useIntelligenceReview), engines, approval workflow
// and stored outputs are untouched — only the surrounding chrome is now supplied
// by the host. backHref/label are still derived from the pathname so PW returns
// to #intelligence and real projects return to Analysis.
...
export function SurveyFindingsReader() {
