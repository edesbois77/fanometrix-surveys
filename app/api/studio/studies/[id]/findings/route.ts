...
import { listFindings, createFindingFromProposal, createManualFinding } from "@/lib/studio/study-finding-service";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body?.origin === "analysis_proposal" && typeof body?.proposalId === "string") {
    ...
    return NextResponse.json({ error: "Provide origin 'analysis_proposal' + proposalId, or 'manual' + evidenceRef." }, { status: 400 });
  }
  if (!res.ok) return NextResponse.json({ error: res.error }, { status: res.status });
