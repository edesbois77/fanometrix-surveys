...
BEGIN
  ...
  -- A CASCADE delete from a pending revision reaches here with the parent already
  -- gone; that is legitimate and must not be blocked.
  IF v_effective IS NOT NULL AND v_effective <= now() THEN
    RAISE EXCEPTION
...
CREATE TRIGGER cgrm_freeze_effective
  BEFORE UPDATE OR DELETE ON public.campaign_group_revision_members
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'M211 OK: effective revisions and their members are frozen by trigger.';
END
$assert$;
