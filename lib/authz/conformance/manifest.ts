...
export const CONFORMANCE_MANIFEST: ConformanceItem[] = [
  ...
  { ref: "Q-10-create-commissioned-research", cls: "HELD", description: "Survey Studio Create capability (governed addition): create-commissioned-research is a Q-10 Product Capability separate from Q-09 Create Product Access (does not grant it, Q-35-D09). Initial governed V1 policy is POLICY-DERIVED (no stored direct-grant column): contextual Fanometrix/admin ALLOW; Publisher and other non-admin REFUSE. Fail-closed when unresolved. present-simulations semantics/grant/parity unchanged. Consumed (not authored) by Survey Studio via hasCapability at the trusted write boundary; UI is projection only.", evidence: ["lib/authz/product-access.test.ts", "lib/authz/conformance/invariants.test.ts"] },
  // ── IW-6 delivered: Resource Entitlement & Authorisation model (HELD) ────────
  { ref: "Q-14/Q-15-mech", cls: "HELD", description: "Organisation Resource Entitlement (subject=Organisation; not ownership) vs User Resource Authorisation (narrows, NEVER expands — user access ⟹ org entitled). Per-class independence (Q-16/Q-17: Report≠Data); coverage-not-inheritance (Q-18); ownership/participation is a Policy Input not entitlement (Q-19/F027). Pure model; persistence = migration 170 (presented).", evidence: ["lib/authz/resource-entitlement.test.ts", "lib/authz/conformance/invariants.test.ts"] },
  { ref: "Q-14/Q-15-activate", cls: "HELD", description: "Persistence LIVE (migration 170 applied; constraints/RLS verified). Dual-run mapping semantics: grant→User Resource Authorisation (allow, never expands), ownership-FK→Org Entitlement via explicit governed Policy Input (ownership≠entitlement). Parity on the current population (0 grants / 0 selected-access → no divergence). SG-6 mechanism demonstrated (isolation, narrows-not-expands, per-asset, no participation→Data).", evidence: ["lib/authz/resource-entitlement.test.ts", "lib/authz/conformance/invariants.test.ts"] },
  { ref: "Q-14/Q-15-enforce", cls: "TARGET", description: "AUTHORITATIVE per-asset entitlement enforcement live — REQUIRES §38-reserved concrete resource/scope-membership schemas AND crosses the IW-0 seam / lib/access.ts super-ALLOW boundary; beyond IW-6 authority (presented, not crossed).", closesAt: "IW-6-enforce", evidence: ["lib/authz/conformance/invariants.test.ts"] },
  ...
  { ref: "F058-activate", cls: "HELD", description: "Session/token revocation LIVE (migration 169 applied): login/change-password JWTs carry tv; requireUser enforces version currency (isSessionRevoked → 401, SG-8) via the existing live re-fetch; bump wired at password-change (F059), forced-password-change-set, and disable (F058); fail-safe preserved (legacy tokens = current).", evidence: ["lib/authz/conformance/invariants.test.ts", "lib/authz/session-currency.test.ts"] },
];
